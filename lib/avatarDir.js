...
// #region agent log
(function(){const payload={location:'lib/avatarDir.js',message:'avatarDir resolved',data:{avatarDir,isServerless,__dirname},timestamp:Date.now(),hypothesisId:'H1'};if(process.env.NODE_ENV!=='production')console.error('[avatar]',payload.location,payload.data);const p=path.join(__dirname,'..','.cursor','debug.log'),p2=path.join(__dirname,'..','avatar-debug.ndjson');try{fs.mkdirSync(path.dirname(p),{recursive:true});fs.appendFileSync(p,JSON.stringify(payload)+'\n');}catch(e){}try{fs.appendFileSync(p2,JSON.stringify(payload)+'\n');}catch(e){}try{fetch('http://127.0.0.1:7249/ingest/883d00fc-6419-4636-bf2d-d40db9bb5ee7',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}).catch(()=>{});}catch(e){}}());
// #endregion
